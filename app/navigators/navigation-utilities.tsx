...
export const setRootNavigation = (ref: React.RefObject<NavigationContainerRef>) => {
  for (const method in RootNavigation) {
    const methodKey = method as keyof typeof RootNavigation

    RootNavigation[methodKey] = (...args: any): any => {
      if (ref.current) {
        return ref.current[methodKey](...args)
      }
    }
  }
}
...
export function getActiveRouteName(state: NavigationState | PartialState<NavigationState>): string {
  if (state.index == null) {
    throw new Error(`state index is ${state.index}`)
  }
